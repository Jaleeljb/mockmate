...
export default function MarqueeStrip() {
  ...
  return (
    <div className="border-y border-slate/10 bg-panel/30 py-5">
      ...
      <div className="group relative overflow-hidden">
        ...
        <div className="flex w-max animate-marquee gap-3 group-hover:[animation-play-state:paused]">
          {track.map((tag, i) => (
            <span
              key={`${tag}-${i}`}
              className="whitespace-nowrap rounded-full border border-slate/20 bg-panelLight/60 px-4 py-1.5 text-xs text-paper/60"
            >
              {tag}
            </span>
          ))}
